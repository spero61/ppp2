...
// REVIEW: probably Clock_window class is too big.
// However to divide this class into pieces, techniques regarding to pointers are required.
// Those are going to be covered in Chapters 17-18.
...
class Clock_window : public Window  // Lines_window inherits from Window
{
   ...
   private:
    ...
    string current_time;
    string current_date;
    bool is_date_label_on;
    Vector_ref<Graph_lib::Text> date_labels;
    // widget:
    Button toggle_date_button;  // show and hide date_labels[i]
...
Clock_window::Clock_window(Point xy, int w, int h, const string& title, Point center, int radius)
    : Window{xy, w, h, title}, c{center}, r{radius}, frame{c, r}, pin{c, 8}, is_date_label_on{false},
      // construct/initialize the parts of the window:
      // location, size, name, action (using lambda expressions)
      toggle_date_button{Point{x_max() - 160, 0}, 80, 20, "Toggle Date", [](Address, Address pw) { reference_to<Clock_window>(pw).toggle_date(); }},
      quit_button{Point{x_max() - 70, 0}, 70, 20, "Quit", [](Address, Address pw) { reference_to<Clock_window>(pw).quit(); }} {
    attach(toggle_date_button);
    attach(quit_button);
    ...
    current_date = current_time.substr(0, 10) + ' ' + current_time.substr(20, 4);
    draw_clock_base();
    start();
}
...
void Clock_window::start() {
    while (true) {
        ...
        second_hands[second_hands.size() - 1].set_color(Color{246});
        second_hands[second_hands.size() - 1].set_style(Line_style{Line_style::solid, 2});
        ...
        minute_hands[minute_hands.size() - 1].set_color(Color{212});
        minute_hands[minute_hands.size() - 1].set_style(Line_style{Line_style::solid, 4});
        ...
        hour_hands[hour_hands.size() - 1].set_color(Color{215});
        hour_hands[hour_hands.size() - 1].set_style(Line_style{Line_style::solid, 6});
...
void Clock_window::toggle_date() {
    if (!is_date_label_on) {
        is_date_label_on = true;
        date_labels.push_back(new Text{Point{c.x - 85, c.y + 185}, current_date});
        date_labels[date_labels.size() - 1].set_color(Color{207});
        date_labels[date_labels.size() - 1].set_font(Font::helvetica_bold_italic);
        date_labels[date_labels.size() - 1].set_font_size(28);
        attach(date_labels[date_labels.size() - 1]);
    } else {
        is_date_label_on = false;
        detach(date_labels[date_labels.size() - 1]);
    }
}
