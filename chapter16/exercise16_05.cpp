// move a shape when next button is clicked
...
class Polygon_window : public Window  // Lines_window inherits from Window
{
   ...
   private:
    ...
    // widgets:
    Button next_button;   // when it is clicked, move the shape
...
Polygon_window::Polygon_window(Point xy, int w, int h, const string& title)
    : Window{xy, w, h, title},
      ...
      next_button{Point{x_max() - 230, 0}, 70, 20, "Next", [](Address, Address pw) { reference_to<Polygon_window>(pw).move(); }},
    ...
    draw_menu.attach(new Button{Point{0, 0}, 0, 0, "Pentagon", [](Address, Address pw) { reference_to<Polygon_window>(pw).draw_shape(Shape_type::pentagon); }});
...
void Polygon_window::draw_shape(Shape_type polygon_type) {
    ...
    switch (polygon_type) {
        ...
        case Shape_type::pentagon:
            shapes.push_back(new Regular_polygon{Point{x, y}, size, 5});
            break;
...
void Polygon_window::move() {
    ...
    const int current_x{shapes[shapes.size() - 1].point(0).x};
    const int current_y{shapes[shapes.size() - 1].point(0).y};
    shapes[shapes.size() - 1].move(x - current_x, y - current_y);
    redraw();
}
...
int main() {
    try {
        Polygon_window win{Point{100, 100}, 600, 400, "Move a shape using next button"};
        return gui_main();
    } catch (exception& e) {
