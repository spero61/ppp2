...
// so that when we figure out how to make our own istreams,
...
// Simulate the istream as a Token stream
class Token_stream {
   public:
...
Token Token_stream::get() {
    if (!buffer.empty())  // check if we already have a Token ready
    {
        Token t = buffer.back();
        buffer.pop_back();
        return t;
...
double eval_function(char c) {
    ...
    switch (c) {
        case sqrtfunc:
            ...
            t = ts.get();
            if (t.kind != ')')
                error("')' expected after function: sqrtfunc");
            if (func_args[0] < 0)
        ...
        case powfunc:
            ...
            t = ts.get();
            if (t.kind != ')')
                error("')' expected after function: powfunc");
        ...
        case logfunc:
            ...
            t = ts.get();
            if (t.kind != ')')
                error("')' expected after function : logfunc");
