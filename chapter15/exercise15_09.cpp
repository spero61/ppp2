...
// heights are rounded to nearest 5cm
...
Histogram::Histogram(Point orig_arg, int xlength_arg, int ylength_arg, vector<double> values_arg, vector<double> values2_arg, string title_arg)
    ...
    for (int i = 0; i < values.size(); ++i) {  // width, values[i]
        ...
        int color_int{16 * 7 + (i * 7)};  // https://www.fltk.org/doc-1.3/fltk-colormap.png
        if (color_int > 255)
...
void Histogram::draw_lines() const {
    ...
    Text title_label{Point{orig.x + xlength - 420, orig.y - ylength + 15}, title};
...
int main() {
    try {
        ...
        string file_name = "heights-korean-men.txt";
        ...
        Histogram bg{bp, xlength, ylength, value_set, heights, "Korean men height sampling"};
        win.attach(bg);
        ...
        for (int i = 0; i < value_set.size(); ++i) {
            bg.labels[i].set_font_size(22);
            bg.labels[i].move(30, 0);
            bg.labels2[i].set_font_size(18);
            bg.labels2[i].move(40, -5);  // height
        }
        // number_of_people
        bg.labels[0].move(8, 0);  // start 3 digits
        bg.labels[1].move(4, 0);
        bg.labels[4].move(-10, 0);  // middle 6 digits
        bg.labels[8].move(8, 0);    // end 3 digits
